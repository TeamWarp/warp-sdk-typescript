...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    const forceAPIResponseForClient = async (client: Warp) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new Warp({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new Warp({ logger: logger, apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Warp({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    test('explicit option', () => {
      const client = new Warp({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new Warp({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Warp({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Warp({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Warp({ apiKey: 'My API Key', fetch: testFetch });
