...
export class PayRates extends APIResource {
   ...
   * @param {string} id
   * @param {RequestOptions} [options] - Options to apply to the request, such as headers and an abort signal.
   * @returns {APIPromise<PayRateRetrieveResponse>} A regular or additional pay rate assigned to a worker.
   *
   * @example
   * ```ts
   ...
   * ```
   */
  retrieve(id: string, options?: RequestOptions): APIPromise<PayRateRetrieveResponse> {
...
export interface PayRateRetrieveResponse {
  id: string;
  workerId: string;
  /**
   * Whether the rate is the worker's regular base compensation or an additional rate such as a bonus, commission, or stipend.
...
export declare namespace PayRates {
  export {
    ...
    type PayRateRetrieveResponse as PayRateRetrieveResponse,
