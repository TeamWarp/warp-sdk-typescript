# Warp Hr TypeScript API Library
...
This library provides convenient access to the Warp Hr REST API from server-side TypeScript or JavaScript.
...
```sh
npm install git+ssh://git@github.com:stainless-sdks/warp-hr-typescript.git
```

> [!NOTE]
> Once this package is [published to npm](https://www.stainless.com/docs/guides/publish), this will become: `npm install warp-hr`
...
1. Via the `WARP_HR_LOG` environment variable
...
const client = new WarpHr({
  logger: logger.child({ name: 'WarpHr' }),
...
```ts
import WarpHr from 'npm:warp-hr';
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/warp-hr-typescript/issues) with questions, bugs, or suggestions.
